...
export class Validator {
  ...
  async analyzeWithAI(
    ...
    // AI analysis would use sequentialThinking client here
    // For now, just return the existing validation result
  ...
  private initializeGuards(): Guard[] {
    return [
      ...
      // Access Control Guards (40-49)
      {
        id: 'G-ROLE',
        exitCode: 40,
        check: (content, _phase, role) => this.checkRoleRestrictions(content, role),
      },
      {
        id: 'G-STATE',
        exitCode: 41,
  ...
  private checkRFCFormat(content: string, phase?: string): { valid: boolean; error?: string } {
    ...
    const [, actContent] = actMatch;

    // Check for RFC structure in act section
    const requiredSections = ['Problem', 'Solution', 'Risks', 'Timeline'];
  ...
  private checkWBSApproval(content: string, phase?: string): { valid: boolean; error?: string } {
    ...
    for (const row of wbsRows) {
      // Skip header rows
      if (row.includes('Phase') || row.includes('---')) continue; // eslint-disable-line no-continue
  ...
  private checkTypeCompliance(content: string, phase?: string): { valid: boolean; error?: string } {
    ...
    while (match !== null) {
      ...
      funcMatch = functionRegex.exec(code);
  ...
  private checkTestCompilation(
    ...
    const testCodeRegex =
      /```(?:typescript|javascript|ts|js)[\s\S]*?(?:describe|it|test|expect)\s*\(/g;
    ...
    if (hasTestCode) {
      ...
      if (!describeMatch && !itMatch) {
        return {
          valid: false,
          error: 'Test code found but no proper test structure (describe/it blocks)',
        };
      }
    }
