...
export class WorkflowManager {
  ...
  async addPhaseLabel(issueNumber: number, phase: Phase, type: 'issue' | 'pr' = 'issue'): Promise<WorkflowResult> {
    try {
  ...
  validatePhaseArtifacts(phase: Phase, artifacts: PhaseArtifacts): { valid: boolean; errors: string[] } {
    const errors: string[] = [];
    
    if (phase === 'PLAN' && artifacts.rfc) {
      const requiredFields = ['problem', 'solution', 'risks', 'timeline'];
      const missingFields = requiredFields.filter(field => !artifacts.rfc![field as keyof typeof artifacts.rfc]);
  ...
  getSummary(): {
    ...
    const remainingPhases = currentIndex >= 0 
      ? this.validPhases.slice(currentIndex + 1)
      : this.validPhases;
  ...
  validatePhaseContent(phase: Phase, content: string, changeMetrics?: { linesAdded: number; filesChanged: number }): { valid: boolean; error?: string } {
  ...
  async getCurrentPhaseFromGitHub(issueNumber: number): Promise<Phase | undefined> {
    try {
      ...
      const phaseLabel = labels.find((label: any) => 
  ...
  async addArtifact(taskId: string, name: string, content: any): Promise<void> {
    const state = await this.loadState(taskId);
    if (!state) {
      throw new Error(`Task ${taskId} not found`);
    }
  ...
  private async updateGitHubLabels(
    ...
    try {
      ...
      // Fallback to gh CLI
      if (oldPhase) {
        execSync(`gh issue edit ${issueNumber} --repo ${repository} --remove-label "phase:${oldPhase}"`, { encoding: 'utf8' });
      }
      execSync(`gh issue edit ${issueNumber} --repo ${repository} --add-label "phase:${newPhase}"`, { encoding: 'utf8' });
