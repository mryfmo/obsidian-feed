...
export async function requestUrl(_param: RequestUrlParam): Promise<{ text: string; status: number; headers: Record<string, string>; arrayBuffer: ArrayBuffer; }> {
  return {
    text: '',
    status: 200,
    headers: {},
...
export class TextComponent {
  ...
  setValue(_v:string): TextComponent { return this; }
  onChange(_fn:(v:string)=>void): TextComponent { return this; }
}
...
export class Setting {
  ...
  addToggle(cb: (t: { setValue: (v: boolean) => void; onChange: (fn:(v:boolean)=>void)=>void })=>void) { cb({setValue(){},onChange(){}}); return this; }
  addText(cb: (t: TextComponent)=>void){ cb(new TextComponent()); return this; }
  addDropdown(cb: (component: DropdownComponent) => void){
    const mockDropdown: DropdownComponent = {
      addOption: function(_value: string, _display: string): DropdownComponent { return this; },
      setValue: function(_value: string): DropdownComponent { return this; },
      onChange: function(_callback: (value: string) => void): DropdownComponent { return this; }
    };
    cb(mockDropdown);
    return this;
  }
...
export class FileSystemAdapter {
  ...
  async exists(_p: string) { return false; }
  async mkdir(_p: string) {}
  async writeBinary(_p:string,_d:ArrayBuffer){}
  async readBinary(_p:string){ return new ArrayBuffer(0); }
...
export class Vault {
  ...
  getAbstractFileByPath(_p: string) { return undefined; }
}
...
export interface PluginManifest { id: string; dir: string; }
