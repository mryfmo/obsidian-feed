...
describe("guard tests", () => {
  ...
  const table = fs.readFileSync("tests/spec.yml","utf-8")
  .trim().split("\n").slice(1).map(l=>l.split(","));
  
  describe("guard-table",()=>{
    table.forEach(([id,_,fixture,exp])=>{
      test(id,()=>{
...
test("LOC over limit fails", async () => {
  const { exitCode } = await run("tests/fixtures/loc_over.md");
...
test("token short fails",()=>{expect(run("tests/fixtures/token_short.md")).not.toBe(0);});
test("token long fails",()=>{expect(run("tests/fixtures/token_long.md")).not.toBe(0);});