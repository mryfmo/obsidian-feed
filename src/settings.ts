...
type BooleanSettingKey = keyof Pick<FeedsReaderSettings,
  "showJot" | "showSnippet" | "showRead" | "showSave" | "showMath" |
  "showGPT" | "showEmbed" | "showFetch" | "showLink" | "showDelete"
>;
  ...
  display(): void {
    ...
    new Setting(containerEl)
      ...
      .addText(text => {
        ...
        text.onChange(async (value) => { // Add async for saveSettings
    ...
    new Setting(containerEl)
      ...
      .addToggle(toggle => {
        ...
        toggle.onChange(async (value) => {
          ...
          await this.plugin.saveSettings();
          if (value) new Notice("Auto-save content enabled (Note: Full functionality might be pending).");
        });
      });
    new Setting(containerEl)
      .setName("Save snippet new-to-old")
      .setDesc(`When saving snippets, place new items at the top of the "${SAVED_SNIPPETS_FNAME}" file if enabled (otherwise append at bottom).`)
      .addToggle(toggle => {
    ...
    const buttonSettings: Array<{ key: BooleanSettingKey; name: string; desc: string }> = [
      ...
      { key: "showRead", name: "Show Mark Read/Unread Button", desc: "Show 'Mark as Read/Unread' button." },
      { key: "showSave", name: "Show Save Button", desc: "Show 'Save' button to save item as markdown." },
      { key: "showMath", name: "Enable Math Rendering (Experimental)", desc: "Attempt to render LaTeX math in feed content if present." },
      { key: "showGPT", name: "Show ChatGPT Button", desc: "Show 'Ask GPT' button (requires API Key)." },
      { key: "showEmbed", name: "Show Embed Button (Experimental)", desc: "Show 'Embed' button for item content." },
      ...
      { key: "showDelete", name: "Show Delete/Restore Button", desc: "Show 'Delete/Restore' button." }
    ];
    buttonSettings.forEach(settingDef => { // Renamed 'setting' to 'settingDef' to avoid conflict
      new Setting(containerEl)
      ...
      .addText(text => {
        ...
        text.onChange(async (value) => {
          ...
          if (!isNaN(num) && num > 0) {
            ...
            new Notice("Cache duration reset to default (1440 minutes). Please enter a valid number.");
          }
          await this.plugin.saveSettings();
    ...
    assetPathSetting.settingEl.style.display = (this.plugin.settings.enableAssetDownload ?? false) ? "" : "none";
    ...
    new Setting(containerEl)
      ...
      .then(setting => { // Access descEl after initialization
        const descEl = setting.descEl;
        descEl.appendText("Provide your OpenAI API Key to enable ChatGPT features. ");
        descEl.createEl("strong", { text: "Note: This key is stored locally in plain text in your Obsidian plugin settings."});
        descEl.createEl("br");
        descEl.appendText("Ensure you understand the security implications.");
      })
      .addText((text: TextComponent) => { // Add type TextComponent
        ...
        text.onChange(async (value: string) => { // Add type string
    ...
    new Setting(containerEl)
      ...
      .setDesc("Prompt template for ChatGPT. Use {{content}} as placeholder for the item content. The content sent will be the first ~4000 characters of the item's text (after HTML removal).")
      .addTextArea(text => {
