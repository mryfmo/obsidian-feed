...
import {
  ...
  LEN_STR_PER_FILE,
  FEEDS_META_FNAME,
  FEEDS_ITEMS_CHUNK_FNAME_PREFIX,
  FEEDS_ITEMS_CHUNK_FNAME_SUFFIX,
  OLD_FEEDS_DATA_FNAME_BASE,
} from './constants';
...
export async function loadSubscriptions(
  ...
  try {
    ...
    if (!result.success) {
      ...
      console.error(
        `loadSubscriptions: Invalid subscriptions file format at ${subsPath}. Details:`,
    ...
    result.data.forEach(feedInfo => {
      ...
      if (!feedInfo.folder || !feedInfo.folder.startsWith(`${feedsStoreBase}/`)) {
        console.warn(
          `loadSubscriptions: Feed "${feedInfo.name}" has missing or invalid folder path: "${feedInfo.folder}". Attempting to rebuild default path. This might indicate a past data issue.`
        );
        const folderName =
          feedInfo.name.replace(/[^a-zA-Z0-9_-]/g, '_').substring(0, 50) || `feed_${Date.now()}`;
    ...
    if (listModified) {
      console.warn(
        'loadSubscriptions: Subscription list was modified due to invalid folder paths. The changes will be saved on the next plugin data save.'
      );
    ...
    new Notice(
      `Error loading subscriptions: ${err instanceof Error ? err.message : String(err)}. Check console.`
    );
    return [];
  }
}
    ...
    feedList.forEach(feedInfo => {
      ...
      if (feedStoreData?.items) {
        feedInfo.unread = feedStoreData.items.filter(
          item => item.read === '0' && item.deleted === '0'
        ).length;
      }
...
export async function removeAllFeedDataFiles(
  ...
  try {
    ...
    if (folder instanceof TFolder) {
      const filesToRemove = folder.children.filter(
        (file): file is TFile =>
          ...
          (file.name === FEEDS_META_FNAME ||
            (file.name.startsWith(FEEDS_ITEMS_CHUNK_FNAME_PREFIX) &&
              file.name.endsWith(FEEDS_ITEMS_CHUNK_FNAME_SUFFIX)) ||
      ...
      console.warn(
        `removeFileFragments: Path "${fullFolderPath}" for feed data exists but is not a folder. Cannot remove fragments. This might indicate a problem with plugin data structure.`
      );
    }
  } catch (err: unknown) {
...
function buildContext(plugin: FeedsReaderPlugin, feedName: string): FeedContext | null {
  ...
  const contextMetaInfo: RssFeedMeta = {
    ...
    title:
      feedFromStore?.title ??
      (feedInfoFromList as FeedInfo & { siteTitle?: string })?.siteTitle ??
      feedInfoFromList.name,
  ...
  if (await plugin.app.vault.adapter.exists(oldFormatFilePath)) {
    ...
    try {
      ...
      if (parsed && parsed.items && parsed.name) {
        ...
        console.warn(
          `Old format data for ${feedMeta.name} failed RssFeedContentSchema validation.`,
...
export async function loadFeedsStoredData(
  ...
  try {
    if (!(await vault.adapter.exists(feedFolderPathAbsolute))) {
      console.warn(
        `loadFeedsStoredData: Folder for feed "${feedMeta.name}" not found: ${feedFolderPathAbsolute}. Returning empty structure.`
      );
    ...
    if (await vault.adapter.exists(metaFilePath)) {
      ...
      if (chunkFilePaths.length > 0) {
        ...
        try {
          ...
          console.error(
            `loadFeedsStoredData: Error reading item chunks in parallel for feed "${feedMeta.name}". Details:`,
            error
          );
    ...
    if (loadedFeed) {
      ...
      for (const item of loadedFeed.items) {
        if (!item.id) {
          const baseString =
            item.link ||
            item.title ||
            `${item.pubDate || ''}${(item.content || '').substring(0, 100)}`;
      ...
      console.error(
        `loadFeedsStoredData: Loaded data for feed "${feedMeta.name}" is invalid after attempting all load paths. Details:`,
    ...
    console.error(
      `loadFeedsStoredData: Failed to load data for feed "${feedMeta.name}" from "${feedFolderPathAbsolute}". Details:`,
      error
    );
  ...
  console.warn(
    `loadFeedsStoredData: Critical failure loading "${feedMeta.name}". Returning empty structure.`
  );
  return RssFeedContentSchema.parse({
    name: feedMeta.name,
    title: feedMeta.name,
