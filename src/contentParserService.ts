...
export class ContentParserService {
  ...
  private pickRoot($: CheerioAPI): Element {
    ...
    for (const selector of selectors) {
      ...
      if (element.length > 0 && element.text().trim().length > 100) { // Basic check for content length
  ...
  async htmlToContentBlocks(html: string, baseHref: string): Promise<ContentBlock[]> {
    ...
    const walk = async (el: Element) => {
      ...
      if (/^(header|nav|footer|aside|script|style|noscript|button|form|input|select|textarea)$/.test(tag)) {
        return;
      }
      ...
      if (tag === "img") {
        const src = $el.attr("src") ?? $el.attr("data-src") ?? $el.attr("data-original") ?? ($el.attr("srcset")?.split(",")[0].split(" ")[0]) ?? "";
        if (src) { // Ensure src is not empty
          const absoluteSrc = absolute(baseHref, src); // Resolve URL before pushing
          blocks.push({ type: "image", src: absoluteSrc, alt: $el.attr("alt") ?? "", width: Number($el.attr("width")) || undefined, height: Number($el.attr("height")) || undefined });
        }
        return;
      }
      if (tag === "video" || (tag === "iframe" && /youtube|vimeo|twitter/.test($el.attr("src") ?? ""))) {
        // Prioritize source tag for video if present
        const src = tag === 'video' ? ($el.find("source").first().attr("src") ?? $el.attr("src")) : $el.attr("src");
        if(src) { // Ensure src is not empty
          const absoluteSrc = absolute(baseHref, src); // Resolve URL
          if (tag === "video") {
            blocks.push({ type: "video", src: absoluteSrc, poster: $el.attr("poster") ?? undefined });
          } else { // iframe for youtube/vimeo/twitter
      ...
      if (tag === "iframe") { // Other iframes (e.g., generic embeds)
        $el.attr('src', absolute(baseHref, $el.attr('src') ?? ""));
        blocks.push({ type: "embed", html: $.html($el) });
        return;
      }
      if (tag === "ul" || tag === "ol") {
        const items = $el.children("li").map((_, li) => $(li).text().trim()).get().filter(Boolean); // Filter out empty items
