...
export class FRSearchModal extends Modal {
  ...
  onOpen() {
    ...
    resultsDiv.style.maxHeight = "40vh"; resultsDiv.style.overflowY = "auto";
    resultsDiv.style.border = "1px solid var(--background-modifier-border)"; resultsDiv.style.padding = "0.5em";

    const doSearch = () => {
      const query = searchInput.value.trim();
      resultsDiv.empty();
      if (!query) { resultsDiv.createEl("p", { text: "Please enter keywords." }); return; }
      if (!this.currentFeedName || !this.plugin.feedsStore[this.currentFeedName]) {
        resultsDiv.createEl("p", { text: "Error: Feed data unavailable." }); return;
      }
      const terms = query.toLowerCase().split(/\s+/).filter(term => term.length > 0);
      if (terms.length === 0) { resultsDiv.createEl("p", { text: "Invalid keywords." }); return; }

      const feedData = this.plugin.feedsStore[this.currentFeedName];
      if (!feedData.items) { resultsDiv.createEl("p", { text: "No items in feed." }); return; }
      ...
      if (results.length === 0) { resultsDiv.createEl("p", { text: `No results found for "${query}".` }); }
      else {
        resultsDiv.createEl("p", {text: `${results.length} result(s) found for "${query}":`, cls: "fr-search-summary"});
        results.forEach(item => {
          ...
          let contentSnippet = (sanitizeHTMLToDom(item.content || "").textContent || "").substring(0, 150);
