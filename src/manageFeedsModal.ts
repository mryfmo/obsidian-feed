...
export class FRManageFeedsModal extends Modal {
  ...
  onOpen() {
    ...
    contentEl.createEl("p", { text: "CAUTION: Actions like Purge and Unsubscribe take effect immediately and cannot be easily undone!" });
    ...
    feedListCopy.forEach((feed) => {
      ...
      const feedInfoSpan = feedDiv.createEl("span", { text: `${feed.name} (Unread: ${feed.unread})` });
      ...
      buttonGroup.style.display = "flex"; buttonGroup.style.gap = "0.5em"; buttonGroup.style.flexWrap = "wrap";

      const markAllBtn = buttonGroup.createEl("button", { text: "Mark all read", title: `Mark all items in ${feed.name} as read` });
      markAllBtn.addEventListener("click", async () => { // Use standard listener
        ...
        try {
          ...
          } else if (infoSpan) { // Fallback if meta not found (should not happen)  
            infoSpan.textContent = `${feed.name} (Unread: 0)`;
          }
        } catch (error: unknown) { 
            console.error(`FRManageFeedsModal (Mark All Read): Error for feed "${feed.name}". Details:`, error);
            new Notice(`Error marking all read for "${feed.name}".`); 
        }
        finally { markAllBtn.disabled = false; markAllBtn.textContent = "Mark all read"; }
      });

      const purgeDelBtn = buttonGroup.createEl("button", { text: "Purge deleted", title: `Permanently remove deleted items from ${feed.name}` });
      purgeDelBtn.addEventListener("click", async () => {
        if (!confirm(`PERMANENTLY remove deleted items from "${feed.name}"?`)) return;
        purgeDelBtn.disabled = true; purgeDelBtn.textContent = "Purging...";
        try {
        ...
        } catch (error: unknown) { // Consolidated catch block
          // Errors from plugin method should already have a Notice (comment from original code)
          console.error(`FRManageFeedsModal (Purge Deleted): Error for feed "${feed.name}". Details:`, error);
      ...
      const purgeAllBtn = buttonGroup.createEl("button", { text: "Purge all items", title: `Permanently remove ALL items from ${feed.name} (keeps subscription)` });
      purgeAllBtn.addEventListener("click", async () => {
        if (!confirm(`PERMANENTLY remove ALL items from "${feed.name}"? Subscription remains.`)) return;
        purgeAllBtn.disabled = true; purgeAllBtn.textContent = "Purging...";
        try {
          await this.plugin.purgeAllItems(feed.name); // Use plugin method
          const infoSpan = listEl.querySelector(`#${feedInfoId}`);
          if(infoSpan) infoSpan.textContent = `${feed.name} (Unread: 0)`;
        } catch (error: unknown) { 
            console.error(`FRManageFeedsModal (Purge All): Error for feed "${feed.name}". Details:`, error);
            new Notice(`Error purging all items for "${feed.name}".`);
        }
        finally { purgeAllBtn.disabled = false; purgeAllBtn.textContent = "Purge all items"; }
      });

      const removeBtn = buttonGroup.createEl("button", { text: "Unsubscribe", title: `Unsubscribe and remove all data for ${feed.name}` });
      removeBtn.style.color = "var(--text-error)";
      removeBtn.addEventListener("click", async () => {
        if (!confirm(`PERMANENTLY unsubscribe from "${feed.name}" and delete all its data?`)) return;
        removeBtn.disabled = true; removeBtn.textContent = "Unsubscribing...";
        try {
          ...
          console.error(`FRManageFeedsModal (Unsubscribe): Error for feed "${feed.name}". Details:`, error);
          // new Notice(`Error unsubscribing from "${feed.name}".`); // Optional: if plugin doesn't always show one
          removeBtn.disabled = false; removeBtn.textContent = "Unsubscribe"; // Ensure button is re-enabled on error
        }
      });
    });
  }
